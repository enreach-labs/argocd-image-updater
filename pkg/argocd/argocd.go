...
func getHelmParamNamesFromAnnotation(annotations map[string]string, img *image.ContainerImage) (string, []string) {
	...
	if img.ImageAlias == "" {
		return "image.name", []string{"image.tag"}
	}
	...
	if param := img.GetParameterHelmImageSpec(annotations, common.ImageUpdaterAnnotationPrefix); param != "" {
		...
		return strings.TrimSpace(param), []string{}
	}
	...
	return helmParamName, strings.Split(strings.ReplaceAll(helmParamVersion, " ", ""), ",")
}
...
func SetHelmImage(app *v1alpha1.Application, newImage *image.ContainerImage) error {
	...
	if hpImageSpec != "" {
		...
		if hpImageTag != "" {
			for _, path := range strings.Split(strings.ReplaceAll(hpImageTag, " ", ""), ",") {
				p := v1alpha1.HelmParameter{Name: path, Value: newImage.GetTagWithDigest(), ForceString: true}
				mergeParams = append(mergeParams, p)
			}
		}
	}
