	...
	case ApplicationTypeHelm:
		...
		if strings.HasPrefix(app.Annotations[common.WriteBackTargetAnnotation], common.HelmPrefix) {
			...
			for _, c := range images {
				...
				if len(helmAnnotationParamVersion) == 0 {
					...
					for _, path := range helmAnnotationParamVersion {
						// image-tag annotation is present, so continue to process image-tag
						helmParamVersion := getHelmParam(appSource.Helm.Parameters, path)
						if helmParamVersion == nil {
							return nil, fmt.Errorf("%s parameter not found", path)
						}
						err = setHelmValue(&helmNewValues, path, helmParamVersion.Value)
						if err != nil {
			...
			var appOriginal yaml.Node
			err = yaml.Unmarshal(originalData, &appOriginal)
			if err != nil {
				return nil, err
			}

			// Find the valuesObject node in the YAML structure
			var valuesObject *yaml.Node
			if appOriginal.Kind == yaml.DocumentNode && len(appOriginal.Content) > 0 {
				root := appOriginal.Content[0]
				if root.Kind == yaml.MappingNode {
					valuesObject = findValuesObjectNode(root)
				}
			}
			...
			for _, c := range images {
				...
				if len(helmAnnotationParamVersion) == 0 {
					...
					for _, path := range helmAnnotationParamVersion {
						...
						err = setHelmValue(valuesObject, path, helmParamVersion.Value)
						if err != nil {
							return nil, fmt.Errorf("failed to set image parameter version value: %v", err)
						}
					}
			...
			override, err = marshalWithIndent(&appOriginal, defaultIndent)
		} else {
...
// findValuesObjectNode recursively searches for the valuesObject node in the YAML structure
// following the path: spec -> source -> helm -> valuesObject
func findValuesObjectNode(node *yaml.Node) *yaml.Node {
	...
	for i := 0; i < len(node.Content); i += 2 {
		...
		if key.Value == "spec" && value.Kind == yaml.MappingNode {
			// Found spec, look for source
			for j := 0; j < len(value.Content); j += 2 {
				specKey := value.Content[j]
				specValue := value.Content[j+1]

				if specKey.Value == "source" && specValue.Kind == yaml.MappingNode {
					// Found source, look for helm
					for k := 0; k < len(specValue.Content); k += 2 {
						sourceKey := specValue.Content[k]
						sourceValue := specValue.Content[k+1]

						if sourceKey.Value == "helm" && sourceValue.Kind == yaml.MappingNode {
							// Found helm, look for valuesObject
							for l := 0; l < len(sourceValue.Content); l += 2 {
								helmKey := sourceValue.Content[l]
								helmValue := sourceValue.Content[l+1]

								if helmKey.Value == "valuesObject" {
									return helmValue
								}
							}
						}
